...
## 지금 무엇을 하고 있나
🔄 **진행 중: 물리학 트랙 신설** (2026-06-21~). 대학물리(Serway, `_content/물리학/`) 기반 3번째 트랙. `physics.html`(math.html 복제·초록테마, 엔진 공유), `home.html`에 ⚛카드. 동작=`js/content_physN.js`, 텍스트=`content/physN.json`. 수학과 동일 패턴(슬라이더·tap-step·골든룰 실계산). **원서 읽기**: poppler 설치됨 → Bash `pdftotext`(텍스트)·`pdftoppm -png`(그림→Read). Read 도구 직접 PDF는 100MB 상한·PATH 문제로 불가, 우회 사용.
- ★ **지향점(사령관)**: 이론을 코드(실시간 수치 적분)로 표현 → **자체 물리 엔진 개발 = 콘텐츠 제작**. 동역학 장면은 닫힌 공식 베끼기 금지, 엔진으로 시뮬레이션.
- ✅ **물리 엔진 PhysLab**(js/physlab.js): World.step(dt)=힘합산→a=F/m→반암시적 오일러 적분(substep), 바닥/벽 반발·원-원 탄성충돌, 힘 생성기(일정힘·스프링·저항·마찰·만유인력·전기장 qE·자기력 qv×B), view(월드↔화면). 모든 동역학 장의 단일 코어.
- ✅ **1장 운동학**(content_phys1, 6장면, 닫힌공식=운동학은 본래 그것): 위치·변위 / 속도=x-t기울기(미분 연결) / 가속도=v-t기울기 / 등가속도 v-t넓이=변위(적분 연결) / 자유낙하 / 포물선.
- ✅ **2장 뉴턴 법칙**(content_phys2, 엔진 시뮬): F=ma 실시간 적분(슬라이더 F·m) / 물리엔진 실험실(공 드래그 던지기·충돌·g·반발 슬라이더). 렌더 스캔 에러0.
- ☐ **남은 장(설계됨, 14장 목표)**: 2뉴턴법칙 · 3일·에너지 · 4운동량 · 5회전 · 6중력 · 7진동 · 8파동 · 9유체 · 10열역학 · 11전기장 · 12회로 · 13자기·유도 · 14빛·현대물리. (각 장 content_physN.js+json, 슬라이더 인터랙티브+실계산. 검증=preview physics.html 렌더 스캔 에러0 → 커밋·범프·push.)
- 미적분 원서(Stewart, `_content/미적분학/`)는 추후 수학 트랙 고급 보강에 활용.
