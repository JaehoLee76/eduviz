...
- **★찾아가기 구조 (항목이 수백 개가 될 때 대비, 2026-08-17)**: ①허브 `adp/index.html` = **검색창 + 파트 태그 칩(가이드·통계·머신러닝) + "이어서 보기"(최근 연 실험실, `tutor.js` 가 localStorage `adp_recent` 에 기록)** ②**개념 사전 `adp/topics.html`** = 주제 전체 검색·태그 필터, 누르면 같은 팝업으로 열림. 새 실험실·새 주제를 추가하면 각각 `LABS` 배열과 `topics/index.js` 에 등록하는 것으로 끝난다(검색·필터는 자동). **서비스워커 `CORE_ASSETS` 에 새 파일 추가 + 캐시 버전 올리기**를 잊지 말 것.
