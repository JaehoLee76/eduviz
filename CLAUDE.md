...
- **tapHint = 컨트롤 박스 안 '정적 행'(2026-06-30 변경, 옛 펄스 알약 폐기).** `E.tapHint(x,y,text,pulse)`는 더 이상 캔버스에 점멸 반투명 알약을 그리지 않고, **DOM `#tapHintRow`**(컨트롤 박스 .sl-unit 포맷, 펄스 없음)에 text+키칩을 넣는다. x/y/pulse 인자는 무시(API 호환만). 위치: **슬라이더 장면=슬라이더 바로 밑 줄**(flex-wrap로 박스 내부 아래줄), **슬라이더 없는 장면=박스 단독**(컨트롤 자리). 키 칩(엔진 자동, 작성자는 키 글자 넣지 말 것): ①슬라이더 장면 → 칩 없음(바가 안내) ②연속 시뮬·리셋탭(물리) → 칩 없음, 텍스트 "화면 탭=…" ③단계 애니메이션(tap+슬라이더無+`auto` 플래그 OR 텍스트 `▶`/`↻` 시작) → **`D 다음 · X 처음`** 칩(★`S 자동` 칩은 작동 안 해 2026-06-30 제거). `tapHint`가 옛 표기(`(D)`·`D · 자동 S`)를 자동 strip. CSS·하단예약은 engine.js `injectViewportCSS`(#tapHintRow)·`fitStage`(bot=272/244/222). 기본설명창(.bubble 띠) max-height=108px(한 줄 더). → **새 단계 장면은 enter에서 `this.s`에 `auto:false`를 넣어 칩이 뜨게**(또는 힌트 텍스트를 `▶`로 시작).
