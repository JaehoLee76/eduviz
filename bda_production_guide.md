...
- **46~48장 (PART G, 15장면)** — 완성·배포됨. ADP 시험 대비.
  - 46·47 실기 240분 리허설(설비 고장 예측 데이터 하나를 두 장이 공유해 탐색→전처리→분할→모델링→평가→해석→답안까지 관통)
  - 48 필기 관문(과락의 산술·학습 배분·객관식 순서·서술형 20점·5과목 자가진단)
  - ★ADP 실기는 **R과 Python 중 택일**이며 파이썬 응시가 정식으로 가능합니다(구름 IDE, 패키지 목록 사전 공고). 따라서 R 노출 금지 원칙은 그대로 유지합니다.
  - ★시험 제도 확정 수치는 **필기 180분·80문항+서술형1(20점)·70점 합격·과목별 40% 과락**, **실기 240분·75점 합격**뿐입니다. 그 외(과목별 문항 수 등)는 공표 자료가 아니므로 지어내지 말고 "가정"임을 화면에 밝히세요.
- **49~53장 (과목별 총정리, 25장면)** — 완성·배포됨. 공식 가이드 개념 231개를 세어 보니 커버리지가 40%였습니다(과목별 29~45%). 원리는 깊었으나 **시험이 묻는 명명된 개념·분류·단계 수**가 비어 있었습니다. 그 층만 채우는 장들입니다.
  - **개념 체계도 렌더러 `js/bda_map.js`(`window.BdaMap(E,cfg)`)** — 계산할 수치가 없는 분류·정의는 이 렌더러로 구조를 보여 줍니다. 핵심은 `items[].s`(형제 항목과 무엇이 다른가 한 줄). 이름만 늘어놓으면 체계도가 아닙니다. 글자 자동 축소·말줄임 내장, **11px 하한 보장**(2026-08-02 소수 시작값에서 하한이 뚫리던 버그 수정 완료).
  - 한 장면에서 탭으로 **cfg 전체를 바꿔치기**하면 여러 벌의 체계도를 담을 수 있습니다(51·52·53장 본보기).
- **54~55장 (실기 최신 경향, 10장면)** — 완성·배포됨. **우리가 가진 공식 가이드는 2021년 판**이고 과목 체계는 그대로지만 **실기 출제는 고급 통계 검정·최적화 쪽으로 움직였습니다.** 최근 회차에 나온 것 중 트랙에 없던 것을 채웠습니다: 분산분석(일원·이원)·두 모집단 비율 검정·이항분포 확률·분포 적합도 검정·대응 이분형 검정(54장), 생존분석·분위수 회귀·베이즈 갱신·선형계획법(55장).
  - 경향 근거와 갭 목록: `_content/BDA/_extract/gap_exam_trend.md`
- **과목별 갭 목록**: `_content/BDA/_extract/gap_subj{1,2,3,4,5}.md` — 다음 보강 때 재사용하세요.
