...
**★2단 겹침 판정 (2026-08-02 추가).** 위 `hits()`의 임계값(가로 35%·세로 50%)은 **부분 겹침을 놓칩니다** — 54장에서 두 라벨이 같은 줄에서 일부만 포개진 실제 결함을 이 하니스가 통과시켰고, 스크린샷으로만 잡혔습니다. 그래서 두 단으로 봅니다.
- **1단(합격 기준)**: 위 임계값으로 **0건**이어야 합니다. 이게 통과 조건입니다.
- **2단(탐색용)**: 임계값을 `ox>2 && oy>2`로 낮춰 후보를 뽑습니다. **이건 합격 기준이 아닙니다** — bbox 높이를 폰트 크기로 근사하다 보니 위아래로 붙은 줄이 2~3px씩 걸려 대량의 오탐이 납니다(전 트랙 276장면에서 22건 나오는데 확인해 보면 대부분 정상). **후보를 스크린샷으로 눈으로 확인**해 진짜만 고치세요.
자동 하니스로 통과했다고 끝내지 말고, 새 장면은 대표 상태를 반드시 눈으로 보세요(§7.8).
