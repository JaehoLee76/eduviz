  ...
  { id:'ai15_01',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s;
      ...
      E.big('도구 사용(Tool Use) — 모르면 연장을 든다',
        'LLM 혼자서는 큰 수의 곱셈이나 오늘 날씨처럼 ‘정확한 사실’을 자주 틀립니다 — 말의 패턴으로 추측하니까요. 똑똑한 사람이 암산 대신 계산기를 집어 들듯, 에이전트는 자기 약점을 알고 <b>외부 도구</b>(계산기·검색·코드 실행)를 호출해 정확한 결과를 받아옵니다. 화면의 '+ (23*47) +'과 추측값의 오차를 함께 확인해 보세요.'); }
  },
  ...
  { id:'ai15_02',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s;
      ...
      E.big('ReAct — 추론(Reason)과 행동(Act)을 번갈아',
        'ReAct는 에이전트가 <b>생각(Thought)</b>으로 다음 할 일을 정하고, <b>행동(Action)</b>으로 도구를 부르고, <b>관측(Observation)</b>으로 결과를 받는 일을 답이 나올 때까지 반복합니다. 생각만 하면 환각에 빠지고, 행동만 하면 길을 잃죠 — 둘을 엮어야 합니다. 화면의 '+(330)+' m와 그 2배 '+(330*2)+'는 단계별로 이어집니다.'); }
  },
