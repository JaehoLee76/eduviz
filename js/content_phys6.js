...
(function(){
  ...
  { id:'phys6_01',
    ...
    draw:function(E){ var s=this.s, W=E.W, H=E.H, ctx=E.ctx, Gc=1, M=10, m=1;
      ...
      E.big('만유인력 F = G·M·m / r² = '+F.toFixed(2), '세상의 모든 질량은 서로를 끌어당깁니다 — 두 질량의 곱이 클수록 세지고, 거리의 <b>제곱에 반비례</b>해 약해집니다. r을 2배 하면 F는 ½이 아니라 ¼로 뚝 떨어지죠. 사과를 떨어뜨리는 그 힘이 달도 붙잡고 있다는 뉴턴의 위대한 깨달음.'); }
  },
  ...
  { id:'phys6_05',
    ...
    draw:function(E){ var s=this.s, w=s.w, p=s.p, sun=s.sun, ctx=E.ctx, W=E.W, H=E.H;
      ...
      E.big('v = '+v.toFixed(2)+'  vs  탈출속도 '+vesc.toFixed(2)+'  →  '+(bound?'묶임(궤도로 되돌아옴)':'탈출!(영영 떠남)'), '총 에너지 E = KE + PE(=−GM/r)의 <b>부호</b> 하나가 운명을 가릅니다. E<0이면 중력 구덩이에 갇혀 타원궤도(파랑), E≥0이면 구덩이를 기어올라 탈출(주황). 그 경계인 탈출속도 v=√(2GM/r)에선 무한히 멀리 가 가까스로 속도 0. 로켓도, 빛조차 못 나오는 블랙홀도 모두 이 부호 이야기.'); }
  },
  ...
  { id:'phys6_03_kepler3', branchOf:'phys6_03', ord:1,
    ...
    draw:function(E){ var s=this.s, w=s.w, p=s.p, ctx=E.ctx, W=E.W, H=E.H;
      ...
      E.big('케플러 3법칙: T² ∝ a³  (T='+T.toFixed(1)+', T²/a³='+ratio.toFixed(3)+' 일정)', '공전주기의 제곱은 궤도 반지름의 세제곱에 비례합니다 — <b>케플러 3법칙 T²=(4π²/GM)·a³</b>. 멀리 도는 행성일수록 길도 길고 속도도 느려 한 바퀴가 한없이 오래 걸리죠(해왕성 165년!). 그래도 a를 바꿔도 T²/a³ 비는 늘 똑같은 값(직선 그래프) — 이 한결같은 비가 한가운데 별의 질량을 일러줘, 보이지 않는 별·블랙홀의 무게를 잽니다. 6.1의 만유인력에서 곧장 나오는 결론이고요.'); }
  }
