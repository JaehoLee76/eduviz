...
(function(){
  ...
  { id:'ch22_01',
    ...
    draw:function(E){ var P=E.Plot, t=this.s.deg*D2R, ctx=E.ctx; P.axes();
      ...
      E.big('회전변환 [[cos'+this.s.deg+'°, −sin], [sin, cos]]', '원점 둘레로 θ만큼 회전! 10장 복소수 곱(각의 합)과 똑같은 행렬이에요'); }
  },
  ...
  { id:'ch22_02',
    ...
    draw:function(E){ var P=E.Plot, m=this.s.mode, ctx=E.ctx; P.axes();
      ...
      E.big('대칭변환 — '+labs[m], '거울에 비춘 상! 분홍 점선이 대칭축. 도형이 뒤집혀요(det = −1)'); }
  },
  ...
  { id:'ch22_04',
    ...
    draw:function(E){ var P=E.Plot, o=this.s.order, ctx=E.ctx; P.axes();
      ...
      E.big(o===0?'대칭 ∘ 회전 (회전 먼저)':'회전 ∘ 대칭 (대칭 먼저)', '합성변환 = 행렬 곱! 순서가 바뀌면 결과가 달라요 (AB ≠ BA, 21장)'); }
  },
  ...
  { id:'ch22_05',
    ...
    draw:function(E){ var P=E.Plot, t=this.s.deg*D2R, ctx=E.ctx, M=[[2,1],[1,2]]; P.axes();
      ...
      E.big(aligned?'★ 고유벡터! Mv가 v와 같은 방향 (단지 늘어남)':'Mv가 v에서 방향이 틀어져요', aligned?'변환해도 방향이 안 변하는 특별한 축 = 고유벡터 (여기선 45°·135°)':'45°(또는 135°)로 맞춰 고유벡터를 찾아보세요'); }
  }
