...
(function(){
  ...
  { id:'hyg3_01',
    ...
    draw:function(E){ var s=this.s, ctx=E.ctx, W=E.W, H=E.H, st=s.step; s.t++;
      ...
      ctx.fillStyle=DIM; ctx.font='11px sans-serif'; ctx.textAlign='left'; ctx.fillText('단위작업장소 평면도 (12m × 8m)', px0+6, py0-8);
      // SEG 그룹 배경(step0 이상)
      ...
      src.forEach(function(S){ var x=mapX(S.fx), y=mapY(S.fy);
        ...
        ctx.fillStyle=S.c; ctx.font='10px sans-serif'; ctx.textAlign='center'; ctx.fillText(S.n, x, y+4); });
      // 근로자
      wk.forEach(function(w){ var x=mapX(w.fx), y=mapY(w.fy);
        ...
        if(st===1 && isTop){ // 호흡영역 반경 30cm(도면 스케일)
          ...
          ctx.fillStyle=ORA; ctx.font='9px sans-serif'; ctx.textAlign='center'; ctx.fillText('호흡영역', x, y-Math.max(10,r30)-4); }
        ctx.fillStyle=hi?ORA:segC[w.seg];
        ctx.beginPath(); ctx.arc(x,y,hi?8:6,0,Math.PI*2); ctx.fill();
        ctx.fillStyle=TXT; ctx.font=(hi?'600 ':'')+'10px sans-serif'; ctx.textAlign='center'; ctx.fillText(w.id, x, y+18);
        // step2: 최고노출근로자 거리선 + 값
        if(st===2 && isTop){ var S=src[w.seg];
          ...
          ctx.fillStyle=ORA; ctx.font='10px sans-serif';
      ...
      if(st===1){ var ax=mapX(0.46), ay=mapY(0.46);
        ...
        ctx.fillStyle=BLU; ctx.font='10px sans-serif'; ctx.textAlign='center'; ctx.fillText('지역시료(고정)', ax, ay+22); }
      // ── 우측 설명 패널
  ...
  { id:'hyg3_02',
    ...
    draw:function(E){ var s=this.s, ctx=E.ctx, W=E.W, H=E.H;
      ...
      ctx.fillStyle=ORA; ctx.font='10px sans-serif'; ctx.fillText('펌프', lx+118, ay+30); ctx.fillText(Q.toFixed(2)+' L/min', lx+118, ay-20);
      ...
      ctx.fillStyle=DIM; ctx.font='10px sans-serif'; ctx.fillText('채취 공기량', colx+colw/2, coly1+16);
      ...
      ctx.fillStyle=AMB; ctx.font='600 11px sans-serif'; ctx.textAlign='left'; ctx.fillText('유량 보정 — 비누거품미터(1차 표준, ±1%)', bx+8, by+16);
      // 뷰렛 + 거품
  ...
  { id:'hyg3_03',
    ...
    draw:function(E){ var s=this.s, ctx=E.ctx, W=E.W, H=E.H;
      ...
      ctx.stroke();
      ctx.fillStyle=BLU; ctx.textAlign='left'; ctx.font='11px sans-serif'; ctx.fillText('바탕(공시료) 신호 분포 σ='+sd.toFixed(3), GX(0.3*sd), gy0+4);
      // LOD 선(3σ)
      ...
      ctx.fillText('LOD=3σ', GX(LOD), gy0-12); ctx.font='11px sans-serif'; ctx.fillText(LOD.toFixed(3)+' mg', GX(LOD), gy1+16);
      ctx.fillStyle=GRN; ctx.font='600 12px sans-serif'; ctx.fillText('LOQ=10σ', GX(LOQ), gy0-12); ctx.font='11px sans-serif'; ctx.fillText(LOQ.toFixed(3)+' mg', GX(LOQ), gy1+16);
      // 구간 라벨
  ...
  { id:'hyg3_04',
    ...
    draw:function(E){ var s=this.s, ctx=E.ctx, W=E.W, H=E.H;
      ...
      ctx.stroke();
      ctx.fillStyle=BLU; ctx.textAlign='left'; ctx.font='11px sans-serif'; ctx.fillText('대수정규분포(측정값의 자연스러운 모양)', GX(mode)+6, gy0+40);
      ...
      function vline(x,c,lab,yo){ ctx.strokeStyle=c; ctx.lineWidth=1.8; ctx.setLineDash([5,4]);
        ...
        ctx.fillStyle=c; ctx.font='600 11px sans-serif'; ctx.textAlign='center'; ctx.fillText(lab, GX(x), gy0-10+yo); }
      ...
      ctx.fillStyle=ORA; ctx.font='600 11.5px sans-serif'; ctx.fillText('= '+z.toFixed(2), qx+220, qy+64);
      ctx.fillStyle=TXT; ctx.font='11.5px sans-serif';
