...
(function(){
  ...
  { id:'ai2_02',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s;
      ...
      E.big('스케일링 — 정규화 vs 표준화', '특징마다 단위·범위가 다르면(나이 vs 연봉) 큰 값이 거리·경사하강을 지배합니다. 그래서 같은 눈금으로 맞춥니다 — <b>정규화</b>는 [0,1]로, <b>표준화</b>는 평균0·표준편차1로 바꿉니다. 스케일링은 거의 모든 모델의 필수 전처리입니다.'); }
  },
  ...
  { id:'ai2_03',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s;
      ...
      E.big('범주형 인코딩 · 결측치', '모델은 숫자만 먹습니다. 범주형(도시·색)은 <b>원-핫 인코딩</b>으로 각 범주를 0/1 열로 펼칩니다(숫자 1·2·3을 주면 가짜 순서가 생기니까요). 빈 칸(결측치)은 버리거나 <b>평균·중앙값·최빈값</b>으로 채웁니다.'); }
  },
  ...
  { id:'ai2_04',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s;
      ...
      E.big('피처 엔지니어링 — 좋은 특징이 모델 절반', '왼쪽 두 고리는 직선으로 못 가릅니다. 그런데 <b>새 특징 r=√(x²+y²)</b> 하나를 만들면, r 임계값 하나로 <b>완벽히 분리</b>되죠. 복잡한 모델보다 <b>좋은 특징</b>이 더 강력할 때가 많습니다 — 이것이 피처 엔지니어링입니다.'); }
  },
  ...
  { id:'ai2_05',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s;
      ...
      E.big('데이터 양과 성능 — 학습 곡선', '<b>데이터는 많을수록 좋습니다.</b> 훈련 데이터 N을 늘리면 모델이 잡음에 덜 휘둘려 검증오차가 줄고 안정됩니다. “더 똑똑한 모델”보다 “더 많은 좋은 데이터”가 성능을 끌어올릴 때가 많죠 — 그래서 데이터 수집·정제가 AI의 절반입니다.'); }
  }
