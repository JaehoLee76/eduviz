...
(function(){
  ...
  { id:'ch3_03',
    ...
    draw:function(E){ var s=this.s, ctx=E.ctx; if(s.play){ s.p+=0.012; if(s.p>=1){s.p=1;s.play=false;} }
      var mp=ez(s.p), u=Math.min(34,E.H*0.052), xl=4*u, t=3*u, big=xl+t, ox=E.W/2-big/2, oy=E.H*0.30;
      box(ctx,ox,oy,xl,xl,'#7ab8ff','x²',16);
      box(ctx,ox+xl,oy,t,xl,'#8fe3b5','3x');
      box(ctx,ox,oy+xl,xl,t,'#8fe3b5','3x');
      if(mp<0.02){ ctx.setLineDash([5,4]); ctx.strokeStyle='rgba(255,255,255,0.3)'; ctx.lineWidth=1.5; ctx.strokeRect(ox+xl,oy+xl,t,t); ctx.setLineDash([]); }
        ...
        var la=(s.p>=1)?E.blink():ca; ctx.globalAlpha=la; ctx.fillStyle='#ffb27a'; ctx.font='700 16px sans-serif'; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText('+9',ox+xl+t/2,oy+xl-dy+t/2); ctx.textBaseline='alphabetic'; ctx.globalAlpha=1; }
      if(s.p>=1){ var sl=E.blink(); ctx.fillStyle='#ffb27a'; ctx.font='600 15px sans-serif'; ctx.textAlign='center';
        // 위: x | 3,  왼쪽: x | 3  → 한 변 x+3 직관
