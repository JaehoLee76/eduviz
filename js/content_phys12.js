...
(function(){
  ...
  { id:'phys12_01',
    ...
    draw:function(E){ var s=this.s, W=E.W, H=E.H, ctx=E.ctx, R=3;
      ...
      // 전류 방향 표시 + 전지/저항 이름표
      ctx.fillStyle=DIM; ctx.font='12px sans-serif'; ctx.textAlign='center'; ctx.fillText('전류 I = '+I.toFixed(2)+' A →', x+w/2, y-10);
      ctx.fillStyle=ORA; ctx.textAlign='right'; ctx.fillText('전지 V='+s.V+'V', x-8, y+h/2-18);
      ctx.fillStyle=GRN; ctx.textAlign='left'; ctx.fillText('저항 R='+R+'Ω', x+w+16, y+h/2);
  ...
  { id:'phys12_05',
    ...
    draw:function(E){ var s=this.s, W=E.W, H=E.H, ctx=E.ctx;
      ...
      // τ에서 63% 기준선(충전 곡선이 τ에 도달하는 높이)
      ctx.strokeStyle='rgba(122,184,255,0.4)'; ctx.setLineDash([3,3]); var vy63=gy0-0.632*gh*0.92; ctx.beginPath(); ctx.moveTo(gx0,vy63); ctx.lineTo(gx1,vy63); ctx.stroke(); ctx.setLineDash([]);
      ctx.fillStyle=BLU; ctx.fillText('0.63V (t=τ='+tau.toFixed(1)+'s)', gx0+6, vy63-4);
      ...
      E.big((s.charging?'충전 중':'방전 중')+' — Vc = '+Vc.toFixed(2)+' V,  τ = RC = '+tau.toFixed(1)+' s', '축전기는 저항을 통해 <b>지수 곡선을 그리며</b> 충전·방전됩니다. 처음엔 벌컥 차오르다가, 차오를수록 점점 더디게 — 가득 찬 컵에 물을 더 붓기 어렵듯, 남은 전압차가 줄어드니 흐름도 줄어듭니다. <b>시정수 τ=RC</b>가 그 속도를 정합니다 — τ 동안 약 63%까지 도달. R이나 C가 클수록 느려집니다. 카메라 플래시가 충전될 때 잠깐 기다리는 그 시간, 신호 필터, 깜빡이 타이머가 모두 이 곡선입니다.'); }
  },
