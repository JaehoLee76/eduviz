...
(function(){
  ...
  { id:'ai9_01',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s;
      ...
      E.big('RNN 구조 — 은닉상태가 기억을 이어 나른다', 'RNN은 시퀀스를 <b>한 단어씩</b> 읽습니다. 매 시점 같은 셀이 “지금 입력 x와 직전 기억 h를 섞어 새 기억을 만든다” — <b>h_t = tanh(Wx·x_t + Wh·h_{t-1} + b)</b>. 핵심은 같은 가중치(Wx·Wh·b)를 <b>매 시점 재사용</b>하고, h가 과거 전체의 요약을 담아 다음으로 전달된다는 점입니다. 화면의 h값은 전부 누적 계산한 것입니다.'); }
  },
  ...
  { id:'ai9_02',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s;
      ...
      E.big('다음 토큰 예측 — ‘Hello’ 자동완성', '언어 모델 RNN은 매 시점 “<b>다음에 올 글자(토큰)</b>는 무엇일까?”를 예측합니다. 은닉상태로 지금까지의 맥락을 요약하고, 마지막에 어휘 전체에 대한 점수(로짓)를 낸 뒤 <b>Softmax</b>로 확률로 바꾸죠. 정답은 따로 라벨링할 필요 없이 <b>시퀀스 자신</b>이 알려 줍니다(다음 글자가 곧 정답) — 이것이 자기지도학습입니다. 화면의 확률은 로짓을 Softmax한 값입니다.'); }
  },
  ...
  { id:'ai9_03',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s;
      ...
      E.big('펼친 RNN — 같은 가중치를 반복해 순전파', '순환 구조를 시간 방향으로 <b>펼치면(unroll)</b> 같은 셀이 시점 수만큼 늘어선 깊은 망이 됩니다. 단, 모든 시점이 <b>똑같은 Wx·Wh·b</b>를 공유하죠(가중치 묶기). 그래서 길이가 다른 문장도 같은 셀로 처리할 수 있습니다. Wh를 키우면 직전 기억이 강하게 전달돼 h가 누적되고, 0에 가까우면 금방 잊습니다 — 화면의 h는 매 시점 펼쳐 계산한 값입니다.'); }
  },
  ...
  { id:'ai9_04',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s;
      ...
      E.big('기울기 소실 — 긴 시퀀스에서 초기 기억이 흐려진다', '학습은 마지막 손실의 기울기를 시간 거꾸로 흘려보내며(BPTT) 가중치를 고칩니다. 그런데 한 칸 거슬러 갈 때마다 <b>tanh′·Wh</b>가 곱해지는데, 이 값이 대개 1보다 작아 <b>곱이 길이만큼 지수적으로 작아집니다</b>. 결국 먼 과거의 토큰일수록 그래디언트가 0에 수렴 — RNN이 <b>장기 의존</b>을 못 배우는 이유죠. 화면의 막대·수치는 곱 ∏ tanh′·Wh 입니다.'); }
  },
