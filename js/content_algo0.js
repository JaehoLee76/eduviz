...
(function(){
  ...
  // ── 공용 '해설 장면' 렌더러 (LLM·팔란티어 등 시스템/실무 심화 분기가 데이터만으로 사용) ──
  // cfg = { title, sub,
  //   code:[문자열...], actLine:정수, codeTitle:'파일명',           // 좌측 코드 패널(선택)
  //   boxes:[{x,y,w,h,c,t,s}...],                                    // 다이어그램 상자(좌표=W/H 비율 0~1, t=제목, s=부제)
  //   arrows:[{x1,y1,x2,y2,c,dash}...],                             // 화살표(좌표=비율)
  //   calc:[{k:'라벨', v:'값(호출부에서 실제 계산)', c:색}...],       // 골든룰: v는 반드시 draw에서 계산한 값
  //   note:'하단 한 줄' }
  window.AlgoDoc = function(E, cfg){ var ctx=E.ctx, W=E.W, H=E.H, TXT='#dfeefb', DM='#9b99a3';
    function RR(x,y,w,h,r){ if(ctx.roundRect){ctx.beginPath();ctx.roundRect(x,y,w,h,r);}else{ctx.beginPath();ctx.rect(x,y,w,h);} }
    ctx.textBaseline='alphabetic';
    if(cfg.title){ ctx.fillStyle=TXT; ctx.font='600 18px sans-serif'; ctx.textAlign='center'; ctx.fillText(cfg.title, W/2, H*0.085); }
    if(cfg.sub){ ctx.fillStyle=DM; ctx.font='13px sans-serif'; ctx.textAlign='center'; ctx.fillText(cfg.sub, W/2, H*0.085+21); }
    var topY=H*0.15;
    // 좌측 코드 패널
    if(cfg.code && cfg.code.length){
      ...
      ctx.fillStyle='rgba(255,255,255,0.03)'; ctx.strokeStyle='rgba(122,184,255,0.30)'; ctx.lineWidth=1;
      RR(cx,topY,cw,ch,8); ctx.fill(); ctx.stroke();
      var yy=topY+pad+(cfg.codeTitle?22:0)+11;
      if(cfg.codeTitle){ ctx.fillStyle='#7ab8ff'; ctx.font='600 12px sans-serif'; ctx.textAlign='left'; ctx.fillText(cfg.codeTitle, cx+pad, topY+pad+11); }
    ...
    // 골든룰 계산값 — 하단 가로 바(중앙정렬·자동 줄바꿈). 상자/코드와 겹치지 않음.
  ...
  { id:'algo0_00', cinematic:true, introCard:true,
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, fr=E.frame, s=this.s;
      ...
      var ANIM=1080, FADE=18, HOLD=180; var _n=(typeof performance!=="undefined"&&performance.now)?performance.now():0, _dt=s.last?(_n-s.last):16.7; if(_dt<0||_dt>200)_dt=16.7; s.last=_n; s.acc=(s.acc||0)+_dt*0.036; var local=s.acc;
