...
(function(){
  ...
  { id:'py9_02',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s, N=s.n;
      ...
      E.tapHint(W/2, H*0.96, '슬라이더로 표본 수를 바꿔 보세요 — 점 구름과 상관 r이 실측됩니다', true);
      E.big('산점도 plt.scatter — 관계를 점으로', '두 변수가 어떻게 함께 움직이는지는 표로 노려봐도 안 보이지만, plt.scatter로 점을 흩뿌리면 즉시 드러납니다 — 우상향이면 양의 상관, 색을 그룹으로 칠하면 군집까지 한눈에. 화면의 상관계수 r과 추세선은 점들에서 실제로 계산한 값입니다. “먼저 그려 보라”는 격언이 가장 빛나는 그림이죠.'); }
  },
  ...
  { id:'py9_04',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s;
      ...
      codePanel(E, W*0.04, H*0.13, W*0.44, code, 'subplots.py');
