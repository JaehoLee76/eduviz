...
(function(){
  ...
  { id:'phys11_01',
    ...
    draw:function(E){ var s=this.s, W=E.W, H=E.H, ctx=E.ctx, k=9, q1=1;
      ...
      E.big('쿨롱 힘 F = k·q₁q₂/r² = '+F.toFixed(2)+'  ('+(s.q2===0?'전하 없음':(repel?'척력':'인력'))+')', '머리카락이 곤두서게 만드는 그 힘입니다. 전하 사이의 힘은 중력과 똑 닮은 1/r² 꼴 — <b>쿨롱 법칙 F=kq₁q₂/r²</b>. 다만 부호가 있어 <b>같은 부호는 밀고(척력), 다른 부호는 당깁니다(인력)</b>. 중력보다 훨씬 강하지만(전자-양성자 간 ~10⁴⁰배) +와 −가 상쇄돼 평소엔 숨어 있습니다. k = 9×10⁹ N·m²/C².'); }
  },
  ...
  { id:'phys11_04',
    ...
    draw:function(E){ var s=this.s, W=E.W, H=E.H, ctx=E.ctx, k=9, Q=1;
      ...
      E.big('전위 V = kQ/r = '+V.toFixed(2)+' V', '전위는 전기의 "높이"입니다 — 산비탈의 높이가 공을 굴리듯, 단위 전하가 갖는 위치에너지(V=kQ/r). +전하 둘레는 솟은 언덕이라 다른 +전하는 아래로 굴러 내려가듯 밀려납니다. 등전위선(파란 원)은 같은 높이의 등고선 — 그 위에서 움직이면 일이 0. 전기위치E U = qV. 1/r²인 힘(장)보다 전위는 1/r로 더 완만합니다(전기장은 이 높이의 기울기).'); }
  },
