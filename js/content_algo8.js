...
(function(){
  ...
  { id:'algo8_08', concept:true,
    enter:function(E){ this.s={}; E.setOn([]); },
    draw:function(E){
      window.AlgoMap(E, {
        title:'세계 수준 LLM을 만든다는 것',
        sub:'데이터에서 서빙까지 — 프론티어 모델 개발의 전 과정',
        stages:[
          {c:'#7ab8ff', t:'① 데이터·사전학습', items:['데이터 수집·정제 · 토크나이저','트랜스포머 셀프 어텐션','스케일링 법칙 (토큰 ≈ 20·N)','학습 루프 + 3D 병렬']},
          {c:'#ffb27a', t:'② 정렬·평가', items:['SFT 지도 미세조정','RLHF · DPO 선호 학습','안전·레드팀','벤치마크와 평가의 어려움']},
          {c:'#8fe3b5', t:'③ 스케일업·서빙', items:['컴퓨트 비용 C ≈ 6·N·D','인프라 고장 · loss spike','KV 캐시 · 연속 배칭','양자화 · 추측 디코딩']} ],
        foot:'데이터 → 사전학습 → 정렬 → 평가 → 서빙 — 코드 몇십 줄보다 이 흐름 전체를 멈추지 않게 돌리는 일이 어렵습니다.' });
    } },

  { id:'algo8_09', concept:true,
    enter:function(E){ this.s={}; E.setOn([]); },
    draw:function(E){ window.AlgoMap(E, {
      title:'온톨로지 — 세계를 모델링해 결정을 자동화한다',
      sub:'객체·관계·액션으로 기업의 세계를 코드화하는 의미 계층',
      stages:[
        {c:'#7ab8ff', t:'① 세계를 객체·관계로', items:['객체(Object) — 펌프·주문·고객','속성(Property) — 온도·상태·금액','관계(Link) — 설비—센서, 고객—주문','테이블이 아니라 사물의 그래프']},
        {c:'#ffb27a', t:'② AI·규칙을 결합', items:['LLM이 같은 모델 위에서 질의·제안','규칙엔진·최적화가 판단을 보조','사람과 AI가 하나의 세계를 공유','정의된 객체·액션만 다루므로 안전']},
        {c:'#8fe3b5', t:'③ 행동으로 되먹임', items:['액션(Action)이 현실에 write-back','데이터→판단→행동→다시 데이터','분석이 아니라 운영 — 디지털 트윈','루프가 닫히면 결정이 자동화됩니다']} ],
      foot:'분석으로 끝나지 않고 행동까지 — 데이터·의사결정·실행을 한 루프로 묶는 것이 온톨로지의 본질입니다.' });
    } },
