/* C++ 제14장 — 스마트 포인터·RAII (RAII · unique_ptr · shared_ptr · weak_ptr · 자원관리 규칙)
   ...
   진짜 컴파일 가능한 표준 C++(C++11/14/17) 코드 + 실계산 결과. 자원 관리 실무 기반. */
(function(){
  ...
  { id:'cpp14_01',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s;
      ...
      var actMap=[7,8,9,10];
      var codeBot=codePanel(E, W*0.04, H*0.10, W*0.46, code, 'raii.cpp', actMap[s.step]);

      // 우측: 자원 상태 + 획득/반납 타임라인 (우측영역 x∈[0.56W,0.96W])
      var bx=W*0.56, ty=H*0.09;
      var open=(s.step>=1 && s.step<=2);   // 자원(파일) 열려 있는가
      var freed=(s.step>=3);
      ctx.textAlign='center';
      // 자원 아이콘
      var rcol = open?GRN:(freed?RED:DIM);
      box(ctx, bx+W*0.12, ty, W*0.16, 40, rcol, null, 0, open?'rgba(126,224,176,0.12)':'rgba(255,255,255,0.03)');
      ctx.fillStyle=rcol; ctx.font='600 13px sans-serif';
      ctx.fillText(open?'파일 OPEN':(freed?'CLOSED':'—'), bx+W*0.20, ty+24);

      // 타임라인
      var tlY=ty+78, tlX=bx+W*0.02, tlW=W*0.36;
      ...
      var caps=['블록 { 에 진입 — 아직 자원 없음',
                ...
                '블록 } 이탈 → 소멸자 자동 호출 → fclose()'];
      var cx=bx, cyy=tlY+58;
      ctx.textAlign='left';
  ...
  { id:'cpp14_02',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s;
      ...
      var actMap=[0,2,4,7];
      codePanel(E, W*0.04, H*0.11, W*0.46, code, 'unique_ptr.cpp', actMap[s.step]);

      // 우측: 소유자 p/q + 힙 객체 (우측영역 x∈[0.56W,0.96W])
      var bx=W*0.56, ty=H*0.12, heapX=bx+W*0.22, heapY=ty+70;
      // 힙 객체
      ...
      // p 소유자
      var pOwns = (s.step<=1);
      box(ctx, bx, ty, W*0.12, 32, pOwns?CPB:DIM, 'p', 13, pOwns?'rgba(90,180,232,0.10)':'rgba(255,255,255,0.03)');
      // q 소유자
      var qY=heapY+70;
      var qOwns = (s.step>=2);
      box(ctx, bx, qY, W*0.12, 32, qOwns?CPB:DIM, 'q', 13, qOwns?'rgba(90,180,232,0.10)':'rgba(255,255,255,0.03)');

      // 소유 화살표
      if(pOwns) arrow(ctx, bx+W*0.12, ty+16, heapX, heapY+19, CPB);
      if(qOwns) arrow(ctx, bx+W*0.12, qY+16, heapX, heapY+19, CPB);
  ...
  { id:'cpp14_03',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s;
      ...
      var codeBot=codePanel(E, W*0.04, H*0.11, W*0.46, code, 'shared_ptr.cpp', actMap[s.step]);

      // 우측: 여러 소유자 → 한 객체 + 카운트 (우측영역 x∈[0.56W,0.96W])
      var bx=W*0.56, ty=H*0.10, objX=bx+W*0.22, objY=ty+64;
      // 객체 + 제어블록(카운트)
      ...
      var caps=['a 생성 — use_count() = 1',
                ...
                'c = a 복사 — count = 3 (셋이 공유)',
                'c 소멸 → count = 2 (아직 살아 있음)',
                '마지막 소유자 소멸 → count 0 → delete'];
      ctx.fillStyle=(freed?RED:GRN); ctx.font='600 12px sans-serif';
      ctx.fillText(caps[s.step], bx, ty+220);
  ...
  { id:'cpp14_04',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s;
      ...
      var codeBot=codePanel(E, W*0.04, H*0.12, W*0.46, code, leak?'cycle_leak.cpp':'weak_ptr_fix.cpp', 4);

      // 우측: 두 노드 A,B 상호 참조 (우측영역 x∈[0.56W,0.96W])
      var bx=W*0.60, ay=H*0.14, byy=H*0.44, nw=W*0.13;
      ...
      // 판정 (코드패널 아래, 세 줄이 0.93H 안에)
      ctx.textAlign='left';
      var jy0=Math.min(H*0.92-42, Math.max(codeBot+22, H*0.66)), jy1=jy0+21, jy2=jy1+21;
      if(leak){ ctx.fillStyle=RED; ctx.font='600 13px sans-serif';
        ctx.fillText('순환 참조 → 지역 a,b 가 사라져도 count가 1 밑으로 안 내려감', W*0.05, jy0);
        ctx.fillStyle=RED; ctx.font='12px sans-serif';
        ctx.fillText('서로가 서로를 붙잡아 영원히 count>0 → delete 불가 → 메모리 누수', W*0.05, jy1); }
  ...
  // ══════════ 5. 자원관리 규칙 — raw new/delete vs 스마트 포인터 ══════════
  { id:'cpp14_05',
    enter:function(E){ this.s={step:0,auto:false}; E.setOn([]); },
    tap:function(E){ this.s.step=(this.s.step+1)%2; E.blip(360+this.s.step*90,0.08); },
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s;
      ...
      var codeBot=codePanel(E, W*0.04, H*0.11, W*0.46, code, s.step===0?'why_smart.cpp':'rules.cpp');

      if(s.step===0){
        // raw vs smart 대비표 (우측영역 x∈[0.56W,0.965W]에 3열)
        var bx=W*0.56, ty=H*0.11, cw=W*0.135, rh=32;
        var rows=[
        ...
        var bx=W*0.56, ty=H*0.09, rw=W*0.40;
        var rules=[
        ...
        for(var i=0;i<rules.length;i++){ var y=ty+i*40;
