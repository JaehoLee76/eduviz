...
(function(){
  ...
  function ebars(E, x0, items, maxv){ var ctx=E.ctx, H=E.H, baseY=H*0.74, bh=H*0.42, bw=46, gap=26;
    ...
    ctx.fillStyle='#9b99a3'; ctx.font='13px sans-serif'; ctx.fillText('에너지 (J)', x0+((items.length*(bw+gap))-gap)/2, baseY-bh-22);
  ...
  function block(E,v,b,m,col){ var ctx=E.ctx, px=v.X(b.x), base=v.Y(0)+20, sz=14+m*4;
    ...
    ctx.fillStyle=col; ctx.font='13px sans-serif'; ctx.textAlign='center'; ctx.fillText(m+' kg', px, base-sz/2+4);
  ...
  { id:'phys3_03',
    ...
    draw:function(E){ var s=this.s, w=s.w, b=s.b, ctx=E.ctx, W=E.W, H=E.H;
      ...
      ctx.fillStyle=DIM; ctx.font='13px sans-serif'; ctx.textAlign='center'; ctx.fillText('바닥 (h = 0)', v.X(5), v.Y(floor)+16);
      // 높이 표시선
  ...
  { id:'phys3_04',
    ...
    draw:function(E){ var s=this.s, w=s.w, b=s.b, ctx=E.ctx, W=E.W, H=E.H;
      ...
      E.ctx.fillStyle=DIM; E.ctx.font='13px sans-serif'; E.ctx.textAlign='left'; E.ctx.fillText('거친 바닥 (μ = '+s.mu.toFixed(2)+')', v.X(0), v.Y(0)+40);
  ...
  { id:'phys3_05',
    ...
    draw:function(E){ var s=this.s, w=s.w, b=s.b, ctx=E.ctx, W=E.W, H=E.H;
      ...
      ctx.fillStyle=DIM; ctx.font='13px sans-serif'; ctx.textAlign='right'; ctx.fillText('일률 P (W)', gx0+58, gy0-gh+8); ctx.textAlign='left'; ctx.fillText('시간 t', gx1-30, gy0+14);
      ...
      ctx.fillStyle=PNK; ctx.font='13px sans-serif'; ctx.textAlign='left'; ctx.fillText('P = F·v', gx1-60, gy0-gh+8);
  ...
  { id:'phys3_01_spring', branchOf:'phys3_01', ord:1,
    ...
    draw:function(E){ var s=this.s, w=s.w, b=s.b, ctx=E.ctx, W=E.W, H=E.H;
      ...
      ctx.fillStyle=DIM; ctx.font='13px sans-serif'; ctx.textAlign='right'; ctx.fillText('지면', v.X(12), v.Y(0)+15);
      // 용수철(왼쪽 아래)
      ...
      ctx.fillStyle=GRN; ctx.font='13px sans-serif'; ctx.textAlign='left'; ctx.fillText('v₀ = '+s.v0.toFixed(1)+' m/s, θ = '+s.ang+'°', sx, v.Y(0)-20);
  ...
  { id:'phys3_03_coaster', branchOf:'phys3_03', ord:1,
    ...
    draw:function(E){ var s=this.s, W=E.W, H=E.H, ctx=E.ctx, g=9.8;
      ...
      ctx.fillStyle=ORA; ctx.font='13px sans-serif'; ctx.textAlign='left'; ctx.fillText('출발 높이 H₀(에너지 한계)', v.X(0)+4, v.Y(s.H0)-4);
      // 차
      var cx=v.X(s.x), cy=v.Y(yhere); ctx.fillStyle=GRN; ctx.beginPath(); ctx.arc(cx,cy-6,7,0,7); ctx.fill();
      ctx.fillStyle=GRN; ctx.font='13px sans-serif'; ctx.textAlign='center'; ctx.fillText('v = '+speed.toFixed(1)+' m/s', cx, cy-18);
      // 에너지 막대
      var PE=g*yhere, KE=0.5*speed*speed, tot=PE+KE;
      var bx=W*0.74, baseY=H*0.74, bh=H*0.4, mx=g*s.H0*1.1;
      ctx.fillStyle=DIM; ctx.font='13px sans-serif'; ctx.textAlign='center'; ctx.fillText('단위질량당 에너지 (J/kg)', bx+57, baseY-bh-12);
      [['위치E',PE,BLU],['운동E',KE,GRN],['합',tot,ORA]].forEach(function(it,i){ var x=bx+i*52, hh=Math.min(1,it[1]/mx)*bh;
        ...
        ctx.fillStyle='#dfeefb'; ctx.font='13px sans-serif'; ctx.textAlign='center'; ctx.fillText(it[1].toFixed(1),x+19,baseY-hh-5); ctx.fillStyle=it[2]; ctx.fillText(it[0],x+19,baseY+16); });
