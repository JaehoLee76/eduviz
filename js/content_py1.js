...
(function(){
  ...
  { id:'py1_01',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s;
      ...
      E.big('변수와 타입 — 값에 이름표 붙이기', '파이썬에서 변수는 ‘상자’라기보다 값에 붙인 <b>이름표</b>입니다. x=5 한 줄이면 정수(int), 3.14는 실수(float), "Guido"는 문자열(str), True/False는 불리언(bool)이 되죠. 자바·C와 달리 <b>타입을 미리 선언하지 않습니다</b> — 담는 값에 따라 타입이 정해지고, 같은 이름에 다른 타입을 다시 담아도 됩니다(동적 타이핑). 위 코드는 그대로 Colab에 붙여 넣으면 실행됩니다.'); }
  },
  ...
  { id:'py1_02',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s;
      ...
      E.big('산술과 연산자 — 파이썬을 계산기처럼', '+ − * 는 익숙하죠. 헷갈리는 셋만 익히면 됩니다: <b>/ 는 항상 실수</b>(7/2=3.5), <b>// 는 몫</b>(7//2=3, 소수 버림), <b>% 는 나머지</b>(7%2=1 — 짝수·홀수 판별에 단골), <b>** 는 거듭제곱</b>(2**10=1024). 화면의 결과는 전부 실제로 계산한 값이라, 코드를 그대로 Colab에 붙여 넣어도 같은 숫자가 나옵니다.'); }
  },
  ...
  { id:'py1_05',
    ...
    draw:function(E){ var ctx=E.ctx, W=E.W, H=E.H, s=this.s, dan=s.dan;
      ...
      E.big('첫 프로그램 — 구구단 출력', '드디어 배운 것을 다 모았습니다: <b>변수</b>(dan)에 값을 담고, <b>곱셈</b>(dan*i)으로 계산하고, <b>f-string</b>으로 결과를 문장으로 만들고, <b>for 반복</b>으로 1~9를 한 번에 처리합니다(반복은 다음 장에서 자세히). 슬라이더로 단을 바꾸면 코드의 dan과 오른쪽 출력 표가 함께 바뀌죠 — 표의 모든 곱셈은 실제로 계산한 값입니다. 이 다섯 줄을 그대로 Colab에 붙여 넣으면 똑같이 출력됩니다. 여러분의 첫 프로그램, 축하합니다!'); }
  }
