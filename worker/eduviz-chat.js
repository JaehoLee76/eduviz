  ...
  async fetch(request, env) {
    ...
    if (U.searchParams.get('data')) {
      ...
      if (request.method === 'POST') {
        const rec = { pos: (pbody && pbody.pos) || {}, memos: (pbody && pbody.memos) || {}, chat: (pbody && pbody.chat) || {}, prefs: (pbody && pbody.prefs) || {}, ts: Date.now() };   // prefs = 기기와 무관한 사용자 설정(테마 등)
        if (KV) { try { await KV.put(uKey, JSON.stringify(rec)); } catch (e) {} }
        return J({ ok: true });
      }
      const raw = KV && await KV.get(uKey);
      return J(raw ? JSON.parse(raw) : { pos: {}, memos: {}, chat: {}, prefs: {} });
    }
...
'5) 수식·기호·코드를 묻는 질문(예: "이 수식 어떻게 읽어요?")에는 [화면수식]·[설명]·[코드]에 있는 식을 근거로, 기호 하나하나를 한국어로 소리 내어 읽어 주고(예: W = F·d → "일 더블유는 힘 에프 곱하기 이동 거리 디"), 각 기호의 이름·뜻·단위를 짧게 밝혀라.\n' +
'6) [조작상태]는 학습자가 지금 화면에서 맞춰 둔 슬라이더 값이다. "지금 화면" 관련 질문이면 이 값을 반영해 답하라.\n' +
'7) [과목]이 기술사·기사 등 자격시험 트랙(예: 산업위생관리기술사)이면 시험 대비 관점으로 도와라 — 개념 질문엔 정의→원리(식·근거)→실무 적용→한 줄 결론의 구조로 답하고, "논술로 어떻게 쓰나/면접에서 어떻게 답하나"류 질문엔 답안 뼈대(두괄식·키워드·도식화 포인트)와 감점 요인을 짚어라. [연습문제]·[모범답안]이 있으면 그대로 베껴 주지 말고, 접근법과 핵심 논점을 스스로 세우도록 유도하며 필요한 만큼만 참고하라.';
    const userMsg = '[현재 장면]\n' + ctxStr + '\n\n[학습자 질문]\n' + question;
